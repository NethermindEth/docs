...
``` yaml title="Example request of parity.clearEngineSigner" 
parity.clearEngineSigner()
```
...
Returns a list of transactions currently in the queue. If address is provided, returns transactions only with given
sender address.
...
``` yaml title="Example request of parity.setEngineSignerSecret" 
parity.setEngineSignerSecret(privateKey)
```
