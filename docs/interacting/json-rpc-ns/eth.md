...
Returns the base fee of the next block in wei
...
curl localhost:8545 \
  ...
  --data '{
      ...
      "id": 0,
      "method": "eth_call",
      "params": [transactionCall, blockParameter, stateOverride, blockOverride]
    }'
```
...
Returns the node's historical data availability for routing and capability discovery.
...
Retrieves a transaction RLP by block hash and index
...
Retrieves a transaction RLP by block number and index
...
Returns storage values for multiple slots across multiple accounts in a single request. Total slot count across all addresses must not exceed 1024.
...
curl localhost:8545 \
  ...
  --data '{
      ...
      "params": [requests, blockParameter]
    }'
```
...
Submits a raw transaction and waits for inclusion in a block, returning the receipt or a timeout error.
...
2. `timeoutMs`: _string_ (hex integer)
...
Returns an EIP-191 signature over the provided message

<Tabs>
<TabItem value="params" label="Parameters">

1. `addressData`: _string_ (address)

2. `message`: _string_ (hex data)
...
Signs the transaction using the unlocked sender account and returns the RLP-encoded signed transaction together with the parsed object.
