...
- #### `Blocks.ParallelExecution` \{#blocks-parallelexecution\}
  ...
  Parallelize transaction execution with Block level access lists. Allowed values: `true` `false`. Defaults to `true`.
...
- #### `Blocks.ParallelExecutionBatchRead` \{#blocks-parallelexecutionbatchread\}
  ...
  Use parallel reads with Block level access lists. Allowed values: `true` `false`. Defaults to `true`.
...
- #### `FlatDb.MinCompactSize` \{#flatdb-mincompactsize\}

  <Tabs groupId="usage">
  <TabItem value="cli" label="CLI">
  ```
  --flatdb-mincompactsize <value>
  --FlatDb.MinCompactSize <value>
  ```
  </TabItem>
  <TabItem value="env" label="Environment variable">
  ...
  Minimum compact size (power of 2, floor for hierarchical compaction) Defaults to `2`.
